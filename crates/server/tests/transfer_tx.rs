...
fn tx_addr_dynamic_fee_increase_in_pool() {
    ...
    // were made.
    let expected_fee = MARK_FEE_MIN.mul(MARK_FEE_MULT.pow(2).unwrap()).unwrap();
...
fn tx_addr_dynamic_fee_increase() {
    ...
    for num in 1..10 {
        ...
        let expected_fee = MARK_FEE_MIN.mul(MARK_FEE_MULT.pow(num).unwrap()).unwrap();
        assert_eq!(addr_info.addr_fee, expected_fee);
    ...
    let expected_fee = MARK_FEE_MIN.mul(MARK_FEE_MULT).unwrap();
    assert_eq!(addr_info.addr_fee, expected_fee);
}
...
fn net_fee_dynamic_increase() {
...

    {
        ...
        let fee = MARK_FEE_MIN.mul(MARK_FEE_NET_MULT.pow(tx_count).unwrap());
    ...
    let expected_fee = MARK_FEE_MIN.mul(MARK_FEE_NET_MULT);
