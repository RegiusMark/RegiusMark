...
fn transfer_from_user() {
    ...
    let res = {
        let tx = {
            let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
                ...
                amount: get_asset("100.00000 MARK"),
                memo: vec![],
                script: minter.genesis_info().script.clone(),
            }));
            tx.append_sign(&minter.genesis_info().wallet_keys[3]);
    ...
    let tx = {
        let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
            ...
            amount: get_asset("99.00000 MARK"),
            memo: vec![],
    ...
    assert_eq!(user_1_bal, Some(get_asset("0.00000 MARK")));

    let user_2_bal = minter.chain().get_balance(&user_2_addr.0.into(), &[]);
    assert_eq!(user_2_bal, Some(get_asset("99.00000 MARK")));
    ...
    assert_eq!(minter_bal, Some(get_asset("901.00000 MARK")));
}
...
fn insufficient_balance_caused_by_fee() {
    ...
    let tx = {
        let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
            base: create_tx_header("1001.00000 MARK"),
            from: from_addr.clone(),
            to: (&to_addr.0).into(),
            amount: get_asset("0.00000 MARK"),
            memo: vec![],
            script: minter.genesis_info().script.clone(),
        }));
        tx.append_sign(&minter.genesis_info().wallet_keys[3]);
    ...
    let chain = minter.chain();
    let cur_bal = chain.get_balance(&to_addr.0.into(), &[]);
    assert_eq!(cur_bal, Some(get_asset("0.00000 MARK")));

    let cur_bal = chain.get_balance(&from_addr, &[]);
    assert_eq!(cur_bal, Some(get_asset("1000.00000 MARK")));
}
...
fn insufficient_fee() {
    ...
    let bad_fee = info
        .total_fee()
        .unwrap()
        .checked_sub(get_asset("0.00001 MARK"))
        .unwrap();
    let tx = {
        let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
...
fn insufficient_balance_caused_by_amt() {
    ...
    let tx = {
        let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
            ...
            from: from_addr.clone(),
            to: (&to_addr.0).into(),
            amount: get_asset("500000.00000 MARK"),
            memo: vec![],
            script: minter.genesis_info().script.clone(),
        }));
        tx.append_sign(&minter.genesis_info().wallet_keys[3]);
...
fn memo_too_large() {
    ...
    let tx = {
        let mut tx = TxVariant::V0(TxVariantV0::TransferTx(TransferTx {
            ...
            memo: (0..=regiusmark::constants::MAX_MEMO_BYTE_SIZE)
                .map(|_| 0)
                .collect(),
            script: minter.genesis_info().script.clone(),
        }));
...
fn script_too_large() {
    ...
    let from_script = Script::new(
        (0..=regiusmark::constants::MAX_SCRIPT_BYTE_SIZE)
            .map(|_| 0)
            .collect(),
    );
...
fn tx_addr_dynamic_fee_increase_in_pool() {
    ...
    // were made.
    let expected_fee = MARK_FEE_MIN
        .checked_mul(MARK_FEE_MULT.checked_pow(2).unwrap())
        .unwrap();
...
fn tx_addr_dynamic_fee_increase() {
    ...
    for num in 1..10 {
        ...
        let expected_fee = MARK_FEE_MIN
            .checked_mul(MARK_FEE_MULT.checked_pow(num).unwrap())
            .unwrap();
        assert_eq!(addr_info.addr_fee, expected_fee);
    ...
    let expected_fee = MARK_FEE_MIN.checked_mul(MARK_FEE_MULT).unwrap();
    assert_eq!(addr_info.addr_fee, expected_fee);
}
...
fn net_fee_dynamic_increase() {
...

    {
        ...
        let fee = MARK_FEE_MIN.checked_mul(MARK_FEE_NET_MULT.checked_pow(tx_count).unwrap());
    ...
    let expected_fee = MARK_FEE_MIN.checked_mul(MARK_FEE_NET_MULT);
